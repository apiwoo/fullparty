...
The guard is the party's long-term memory: a server-side judgment ledger + failure catalog, reached through the MCP tools `qa_init` / `qa_scan` / `qa_report` / `qa_triage` / `qa_note` / `qa_status` (connection setup: AGENTS.md). Client-agnostic — call the MCP tools directly; slash commands are optional wrappers.
...
- **Context notes** (`qa_note`): the moment the owner states an operational fact the code cannot show — "that endpoint is called by an external ops tool", "this table is written by a nightly job", "this field stays for old clients" — record it via `qa_note` right then (stable slug; resubmitting a slug updates it). Don't wait for a scan to ask: unrecorded operational history is exactly what gets working code deleted as "unused".
- **Pre-deletion gate** (`qa_status` → `guard_rails`): before deleting "unused"-looking code, removing an API/field/table, or any structural refactor, call `qa_status` and check `guard_rails` (intended / dont_touch / context entries). Anything a rail covers is protected — present it to the owner instead of touching it. "No callers in the repo" is never proof of death: a live service has callers the repo can't see (ops tools, old clients, cron jobs, partner integrations).
...
6. **Back up before you break.** Before destructive or bulk operations: snapshot/backup, move-don't-delete (recoverable `_Legacy/` style), verify references are safe, make batches idempotent so a crash re-runs cleanly. Code that merely *looks* unused is not proven dead — check the guard's rails first (`qa_status` → `guard_rails`, see Guard integration); live services have callers the repo can't see.
